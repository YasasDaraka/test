...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleServiceException(ServiceException exp) {
        ...
        return ResponseEntity.status(status)
                .body(getErrorAttributes(status.value(), status, exp.getMessage()));
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public Map<String, Object> handleDataValidationException(MethodArgumentNotValidException exp) {
        Map<String, Object> errorAttribute = getErrorAttributes(HttpStatus.BAD_REQUEST.value(),
                HttpStatus.BAD_REQUEST, "Data Validation Failed");
